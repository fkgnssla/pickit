        ...
        <div className="border">
          <div className="box">
            {tempVotes.map((tempVote, index) => (
              <div key={tempVote.id} style={{backgroundColor:"rgb(220, 255, 225)", padding:"10px", margin:"10px", borderRadius:"5px"}}>
                <div style={{display:"flex", justifyContent:"space-between"}}>
                    <h3>{index + 1}. {tempVote.title}</h3>
                    <div>
                        <img src={addIcon} className="deleteButton" alt="add" onClick={() => createVote(tempVote.id)}/>
                        <img src={deleteIcon} className="deleteButton" alt="delete" onClick={() => deleteVote(tempVote.id)}/>
                    </div>
                </div>
                <img src={tempVote.thumbnail} alt={tempVote.title} width="100" />
                <span> | 방송사: {tempVote.broadcast_name}</span>
                <span> | 설명: {tempVote.description}</span>
                <br />
                <span>기간: {new Date(tempVote.start_date).toLocaleDateString() + " " + new Date(tempVote.start_date).toLocaleTimeString()} ~ {new Date(tempVote.end_date).toLocaleDateString() + new Date(tempVote.end_date).toLocaleTimeString()}</span>
                <hr />
                <h5>후보 목록:</h5>
                <ol>
                  {tempVote.candidates.map((candidate, candidateIndex) => (
                    <li key={candidate.number}>
                      <img src={candidate.profile_img} alt={candidate.name} width="50" />
                      <span> | 후보 {candidateIndex + 1}: {candidate.name}</span>
